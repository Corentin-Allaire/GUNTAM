...
class TestBatchedBeamSearchSeedReconstruction:
    ...
    def test_score_uses_edge_count_denominator_not_hit_count(self):
        ...
        mask = torch.ones(B, N, dtype=torch.bool)
        edge = _to_edge_batched(att)
        chains, _, best_scores = batched_beam_search_seed_reconstruction(edge, mask, max_chain_length=5, beam_width=5)
        chain = chains[0, 0, :]
...
class TestApplyRadialSeparationFilter:
    ...
    def _build_score(self, chains):
        """Helper to build a dummy score tensor to check it properly get set to -1 when Δρ fails."""
        score = torch.zeros_like(chains[:, :, 0], dtype=torch.float)
        return score
    ...
    def test_incomplete_result_dropped_to_all_minus_one(self):
        ...
        fewer than target_length=3, so the seed score is set to -1."""
        chains = torch.tensor([[[0, 1, 2]]], dtype=torch.long)  # [B=1, N=1, raw_length=3]
        score = self._build_score(chains)
        rho = torch.tensor([[0.0, 10.0, 12.0]])  # Δ(A,B1)=10>5, Δ(B1,B2)=2<5
        out, score = apply_radial_separation_filter(chains, score, rho, min_delta_rho_mm=5.0, target_length=3)
    ...
    def test_two_kept_completes_when_target_length_is_two(self):
        ...
        score = self._build_score(chains)
        out, score = apply_radial_separation_filter(chains, score, rho, min_delta_rho_mm=5.0, target_length=2)
        assert score[0, 0] != -1.0
    ...
    def test_global_rank_space_gather_uses_value_not_position(self):
        ...
        out, score = apply_radial_separation_filter(chains, score, rho_rank_space, min_delta_rho_mm=5.0, target_length=3)
        assert score[0, 0] != -1.0
    ...
    def test_check_bounds_true_raises_on_out_of_range_index(self):
        ...
        with pytest.raises(AssertionError):
            apply_radial_separation_filter(chains, score, rho, min_delta_rho_mm=5.0, target_length=3, check_bounds=True)
    ...
    def test_batch_dim_mismatch_raises_immediately(self):
        ...
        score = self._build_score(chains)
        with pytest.raises(AssertionError):
            apply_radial_separation_filter(chains, score, rho, min_delta_rho_mm=5.0, target_length=3)
    ...
    def test_integration_with_real_beam_search_score_never_recomputed(self):
        ...
        rho = torch.tensor([[0.0, 10.0, 12.0, 20.0, 100.0]])
        filtered, score = apply_radial_separation_filter(chains, score, rho, min_delta_rho_mm=5.0, target_length=3)
