    ...
    def test_z0_weight_none_is_backward_compatible(self):
        ...
        for bin_size in [0, 1000]:
            result = _build_good_pairs_tensors(
                ...
                num_bins=1,
                pv_weight=10,
                particles_batch=None,
                z0_weight_bin=bin_size,
            )
            labels = result[0, 0, :, 2]
            non_zero = labels[labels > 0]
...
class TestToTensor:
    def test_basic_conversion(self):
        ...
        hits_tensor, particles_tensor, hit_to_particle_tensor = _to_tensor(
            data_batch, particles_batch, bins, hit_to_particle, hit_features, particle_features, num_bins, max_hits_per_bin
        )
    ...
    def test_single_vs_two_workers_same_result(self, tmp_path):
        ...
        for file_idx in range(num_files):
            ...
            for key in tensor_keys:
                assert torch.equal(data1[key], data2[key]), f"Tensor mismatch in '{key}' for file index {file_idx}"
