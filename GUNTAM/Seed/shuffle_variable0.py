#je dois shuffle les variables des hits une par une 
...
class SeedTransformer(nn.Module):
    ...
    def encodeSpacePoint(self, hits: Tensor, mask: Tensor) -> Tensor:
        ...
        idx = list(range(encoded_hits.size(1))) #mélange par hit au sein d'un bin pour chaque bin
        
        random.shuffle(idx)
        idx = torch.tensor(idx)
        
        encoded_hits[:, :, 0] = encoded_hits[:, idx, 0] #première feature
...
#  #dimensions de encoded_hits :

# hits = data_1event["hits_tensor"]
# mask = data_1event["padding_mask"]

# hits = hits.reshape(-1, hits.shape[-2], hits.shape[-1])   # [1,126,1200,6] -> [126,1200,6]
# mask = mask.reshape(-1, mask.shape[-1])    

# out = model.encodeSpacePoint(hits=hits, mask=mask)

# #dimension encoded_hits : torch.Size([126, 1200, 256]) 
# # 1200 hits
# # 256 features
# # 126 bins

#on print la loss avec shuffle sur la première variable :
