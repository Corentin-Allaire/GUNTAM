...
def train_classifier(train_dataloader, input_shape, hidden_1, hidden_2, hidden_3, hidden_4, output_shape, p, n_epochs, lr, model_save:str, criterion=torch.nn.CrossEntropyLoss().to(device)):

    model_CE = MLP_CE(input_shape=input_shape, hidden_1=hidden_1, hidden_2=hidden_2, hidden_3=hidden_3, hidden_4=hidden_4, output_shape=output_shape, p=p, activation=torch.nn.ReLU()).float()
    optimizer_CE = torch.optim.Adam(model_CE.parameters(), lr)
    model_CE = train_loop_CE(train_dataloader, model_CE, n_epochs, optimizer=optimizer_CE, criterion=criterion, lr=lr, device=device)
...
if __name__ == "__main__":
    ...
    train_classifier(train_dataloader= train_dataloader, input_shape=28, hidden_1=512, hidden_2=256, hidden_3=128, hidden_4=64, output_shape=2, p=0, n_epochs=10, lr=1e-3, criterion="CE", model_save="classifier")
